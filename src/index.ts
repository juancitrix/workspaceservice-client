  ...
  return new Promise<void>((resolve, reject) => {
    ...
    options.headers = {
      ...
      'X-Citrix-AM-CredentialTypes':
        'none, username, domain, password, newpassword, passcode, savecredentials, textcredential, webview',
...
export async function getPrimaryDesktopConnection(
  username: string,
  password: string,
  host: string,
...
function getLaunchUrlFromResponse(response: any): string {
  ...
  const found = response.resources.some((resource: any) => {
    if (
      resource.desktopassignmenttype === 'assigned' &&
      resource.isdesktop === true &&
      resource.subscriptionstatus === 'subscribed'
    ) {
